...
6. `/report`：举报页面。当前只提供入口，页面暂留空。
7. `/reports`：举报受理页面。仅“管理员”和“服主”身份可见，当前只提供入口，页面暂留空。
8. `/status`：服务器状态页面，展示开服状态、磁盘空间、内存占用、备份文件列表、日志 ERROR 情况和在线玩家信息。
...
6. 登录页和注册页会提示玩家：未注册白名单时，直接在 Minecraft 客户端连接服务器即可获取白名单验证码。
