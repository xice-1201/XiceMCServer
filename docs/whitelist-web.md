...
7. `/docs`：服务器文档首页。访客和登录玩家均可见，内容来自运行时 Markdown 文档并渲染为 HTML。未登录访客访问时不展示左侧菜单栏，右上角显示“返回登录”；“管理员”和“服主”登录后可在右上角点击“编辑”进入 Markdown 编辑模式。
...
14. `/permissions`：权限管理页面。仅“管理员”和“服主”可见，可按指令筛选拥有对应权限的玩家，默认展示 `/creative`。添加权限时支持输入玩家 ID 或 UUID 并自动补全，保存后写入对应插件运行时配置并通过 RCON 重载。
