...
Microsoft 身份验证框架已经接入。启用 `MICROSOFT_AUTH_ENABLED=true` 后，登录页和注册页会显示 Microsoft 身份验证入口；填入 Client ID 和回调地址后，完整流程如下：
...
1. 准备域名和 HTTPS。
2. 在 Microsoft 应用注册中配置回调地址，例如 `https://xicemc.site/auth/microsoft/callback`。
