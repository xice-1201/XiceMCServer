...
1. `/`：公开首页。未登录访客看到个人技术开发随记、目录、技术实现、插件动态、运维记录和更新日志；右上角提供统一的“测试入口”折叠弹层。
2. `/register`：兼容入口，展示同一套公开首页与测试入口。玩家填写玩家ID，以及 5 分钟内有效的邀请码后，Web 服务写入服务器 `whitelist.json`，再通过本机 RCON 执行 `whitelist reload` 使白名单立即生效。
3. `/home`：登录后的首页，以固定比例个人 ID 卡形式展示玩家皮肤、身份、个人简介、UUID、注册时间、累计游玩时间和上次登出地点，并提供编辑个人简介和修改邀请码入口。按钮下方展示该玩家拥有的领地和被授权的领地，数据来自 `XiceClaim` 运行时 `claims.yml`。编辑个人简介通过首页对话框完成。
4. `/profile`：个人简介保存接口。玩家在首页对话框中提交后，Web 会更新展示在个人 ID 卡上的简介，默认简介为“一名普通的Minecraft玩家”。
5. `/password`：修改邀请码页。玩家输入原邀请码和两次新邀请码后更新后续测试入口使用的邀请码。
...
当前测试入口用于白名单玩家进入 Web 后台，也用于尚未加入白名单的玩家提交临时邀请码。处理顺序是：

1. 玩家填写的玩家ID格式合法。
2. 如果该 ID 已存在于服务器 `whitelist.json`，则把玩家填写的邀请码按 Web 用户邀请码校验。
3. 如果该 ID 不在服务器 `whitelist.json`，则把玩家填写的邀请码按 `XiceTextArranger` 生成的白名单临时邀请码校验。
4. 校验通过后浏览器保存签名 Cookie，并进入 Web 后台。

所有成功登记白名单的玩家都会获得默认邀请码 `123456`。邀请码摘要保存在 PostgreSQL 的 `web_players.password_hash` 字段中，当前按项目要求使用 MD5 摘要保存。
玩家登录后可以在首页点击“修改邀请码”，新邀请码长度不得低于 `6` 位，且只能包含英文、数字和下划线。

`web_players.role` 用于记录玩家 Web 身份。默认身份为“玩家”；服主账号可设置为“服主”；另有“管理员”身份。`web_players.profile_bio` 用于记录玩家个人简介，默认值为“一名普通的Minecraft玩家”。举报受理菜单仅对“管理员”和“服主”可见。
...
白名单临时邀请码由 `XiceTextArranger` 在玩家被白名单拒绝时生成：

1. 邀请码有效期为 `5` 分钟。
2. 同一玩家在邀请码过期前重复尝试进服，会继续得到同一个邀请码。
3. 邀请码在 Web 登记成功使用后会立刻失效。
4. 邀请码文件默认位于 `/opt/xicemc/runtime/plugins/XiceTextArranger/verification-codes.tsv`。
5. Web 登记不会执行 `whitelist add` 命令，避免 Paper 拒绝异步执行该命令；登记流程直接写入白名单文件并刷新白名单。
6. 测试入口位于公开首页右上角，玩家可通过“测试入口”提交邀请码。
