...
func TestPublicTemplatesExecute(t *testing.T) {
	...
	for _, name := range []string{"public", "publicTech", "publicPlugins", "pluginXiceClaim", "pluginXiceAuditLog", "pluginXiceCommandControl", "pluginXiceTextArranger", "publicOps", "publicChangelog"} {
		if err := templates.ExecuteTemplate(&bytes.Buffer{}, name, data); err != nil {
			t.Fatalf("ExecuteTemplate(%s) error = %v", name, err)
		}
	}
